...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if batch_idx == 0 or (batch_idx + 1) % 5 == 0 or batch_idx == len(train_loader) - 1:
            ...
            accelerator.print(f'[{epoch+1}/{config.train.epochs}][{batch_idx+1}/{len(train_loader)}] train loss: {loss.item():.10f} | lr: {lr:.10f} | grad norm: {grad_norm}')
        ...
        # validation
        if config.train.val_interval > 0 and (batch_idx + 1) % config.train.val_interval == 0:
    ...
    # epoch summary
    accelerator.print(f'[{epoch+1}/{config.train.epochs}] train epoch loss: {losses.avg:.10f}')
...
def valid_one_epoch(
    ...
    # epoch summary
    accelerator.print(f'[{epoch+1}/{config.train.epochs}] {prefix} epoch loss: {losses.avg:.10f}')
